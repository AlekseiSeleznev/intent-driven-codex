...
After installation, review and edit the project context files that the installer
created or preserved:

```bash
ls -la CONSTITUTION.md ARCHITECTURE.md CONTEXT.md .secrets.example.env 2>/dev/null || true
${EDITOR:-vi} CONSTITUTION.md ARCHITECTURE.md
```

Do not blindly overwrite project-owned context. Copy `CONSTITUTION.md`,
`ARCHITECTURE.md`, `CONTEXT.md`, or `.secrets.example.env` from the template
only when the target project does not already have that file, or after explicit
review by the project owner. Fill the constitution with required technologies,
MCP servers, external systems, documentation sources, verification rules, and
additional AI instructions for the target project. Keep `ARCHITECTURE.md` as the
current architecture snapshot and link in-force ADRs from it. Create the local
secret file yourself when needed:
...
If you do not want to use the installer, copy only these overlay-owned paths and
preserve existing project-owned files unless the project owner explicitly
approves replacing them:

```text
.codex/prompts/
.codex/skills/
.codex/openspec-artifact-review.json     # non-secret artifact review defaults
openspec/schemas/intent-driven/
openspec/README.md                     # only if absent or after explicit review
scripts/check-overlay
scripts/install-overlay                # optional but recommended for future updates
scripts/openspec-artifact-review
...
Also add or preserve these ignore rules so `scripts/check-overlay` can validate
local-only state safely:
...
The `.codex/session/` directory is created locally as needed by
`scripts/openspec-session-state`; do not copy tracked credentials or session
state into it.
