...
- smoke check validates `proposal/specs/design/review/adr/test-plan/tasks` apply context.
...
`review.md` records mandatory `grill-with-docs` review before ADR planning.
`test-plan.md` records TDD where feasible or the strongest verification-first
substitute before implementation tasks. Existing active changes created with an
older template should add these artifacts explicitly before apply.
