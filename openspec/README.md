...
`review.md` is the mandatory `grill-with-docs` context review gate. `test-plan.md` is the TDD or verification-first planning gate. Both are OpenSpec change artifacts; root `CONSTITUTION.md`, root `ARCHITECTURE.md`, top-level `adr/`, and local secrets remain outside the change artifact graph.
