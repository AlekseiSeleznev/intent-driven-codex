...
The `intent-driven` schema SHALL instruct Codex to account for root `CONSTITUTION.md`, root `CONTEXT.md` or `CONTEXT-MAP.md`, root `ARCHITECTURE.md`, and in-force ADR context during artifact creation, grill, test planning, and apply guidance without making those files OpenSpec change artifacts.
