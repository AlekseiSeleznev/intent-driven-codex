# Tasks: fix-artifact-review-session-config

- [x] Add OpenSpec delta spec for session-effective artifact review config behavior.
- [x] Implement default session-effective config loading in `scripts/openspec-artifact-review`.
- [x] Preserve explicit `--config` priority and add `--raw-config` / `--ignore-session` escape hatch.
