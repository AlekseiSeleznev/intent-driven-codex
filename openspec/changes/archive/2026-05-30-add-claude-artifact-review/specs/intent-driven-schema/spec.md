...
The `intent-driven` schema SHALL instruct Codex to account for root `CONSTITUTION.md`, root `CONTEXT.md` or `CONTEXT-MAP.md`, root `ARCHITECTURE.md`, in-force ADR context, and configured non-secret artifact review reports during artifact creation, grill, test planning, verification, and apply guidance without making root project context files OpenSpec change artifacts.
