...
The template SHALL support a root-level `PROJECT_CONSTITUTION.md` file as Git-tracked, non-secret project context that Codex must treat as persistent rules outside the OpenSpec change artifact lifecycle.

#### Scenario: Constitution survives OpenSpec archive
- **GIVEN** a repository contains `PROJECT_CONSTITUTION.md` at the project root
- **WHEN** Codex archives an OpenSpec change
- **THEN** `PROJECT_CONSTITUTION.md` remains at the project root
...
- **GIVEN** a user updates `PROJECT_CONSTITUTION.md`
...
- **WHEN** the user fills `PROJECT_CONSTITUTION.md`
...
`PROJECT_CONSTITUTION.md` MUST NOT contain secret values and SHALL reference local `.secrets.local.env` variables for credentials needed by listed external systems.
...
- **AND** no login, password, token, or service URL secret value is written into `PROJECT_CONSTITUTION.md`
...
Codex MUST stop and ask the user before continuing when the project constitution conflicts with the requested task, an OpenSpec artifact, or an intended implementation action.

#### Scenario: Task conflicts with required technology
- **GIVEN** `PROJECT_CONSTITUTION.md` requires a specific technology
...
- **GIVEN** an OpenSpec artifact proposes behavior that contradicts Additional AI Instructions in `PROJECT_CONSTITUTION.md`
