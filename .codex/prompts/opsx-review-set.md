...
1. Parse the requested stage and key/value settings. Supported keys include `model`, `effort`, `budget`/`maxBudgetUsd`, `budgetRub`/`maxBudgetRub`, `baseUrl`, `endpoint`, `apiKeyEnv`, `structuredOutputMode`, `blockOn`, and `enabled`.
