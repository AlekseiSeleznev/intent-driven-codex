...
2. If `.codex/openspec-artifact-review.json` exists, generate a temporary effective config with `scripts/openspec-session-state --effective-review-config --out <tmp>` and validate it with `scripts/openspec-artifact-review --validate-config --config <tmp>`.
