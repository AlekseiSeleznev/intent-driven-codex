...
- In the final apply summary, include `Total artifact review cost: <value> RUB` for OpenAI-compatible artifact reviewer calls. Use current-run tracked costs when available; otherwise, when persisted reports exist, run `scripts/openspec-artifact-review --cost-summary --change <name>` and label the result as the persisted invocation/report total for the change. If review ran or reports exist but no numeric cost metadata exists, report `Total artifact review cost: unavailable`.
