...
4. Refuse to apply if planning artifacts are missing or if `openspec/changes/<name>/{proposal.md,design.md,adr.md,tasks.md,specs/**}` has uncommitted changes, unless the user gives an explicit one-time override.
5. For intent-driven changes, read every file in `contextFiles` and then read top-level `adr/*.md` when present. If `contextFiles.adr` points at per-change `adr.md`, still derive durable constraints from top-level ADRs.
