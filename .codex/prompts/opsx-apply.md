...
- If `/opsx:apply` invokes OpenAI-compatible artifact review during implementation, parse each helper JSON output or persisted report and include a user-visible `## Artifact Review` block after every reviewer call; tool output alone is not sufficient.
