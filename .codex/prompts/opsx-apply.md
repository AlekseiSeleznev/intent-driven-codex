...
Artifact Review reporting:
- If `/opsx:apply` invokes OpenAI-compatible artifact review during implementation, parse each helper JSON output or persisted report and include a compact localized user-visible review status block after every reviewer call; tool output alone is not sufficient. Run `scripts/openspec-artifact-review --cost-summary --change "<name>"` after reviewer calls when persisted reports/ledger may exist; totals must include retries/re-runs, not just the latest stage report.
...
- In the final apply summary, include the localized total artifact review cost for OpenAI-compatible reviewer calls using current-run tracked costs when available; otherwise, when persisted reports exist, run `scripts/openspec-artifact-review --cost-summary --change <name>` and label the result as the persisted invocation/report total for the change. If review ran or reports exist but no numeric cost metadata exists, report the localized equivalent of `review cost unavailable`.
