...
- design.md (technical approach and C4-style boundaries when useful)
...
4. **Create artifacts in sequence until apply-ready**
   ...
   a. **For each artifact that is `ready` (dependencies satisfied)**:
      ...
      - Treat this artifact as a checkpoint boundary: request explicit approval before staging/committing or before a dependent artifact consumes uncommitted state, unless the user pre-approved a batch checkpoint workflow
...
- Use `grill-with-docs` for proposal/design review when scope, risk, assumptions, integration impact, or Brownfield context is material. Do not use the plain review skill.
...
- When creating `tasks.md` for an intent-driven change, read proposal/specs/design/per-change `adr.md` and top-level `adr/*.md`; tasks must preserve in-force ADR constraints.
...
**Guardrails**
- Checkpoint after each artifact; mutating git operations require explicit user approval.
