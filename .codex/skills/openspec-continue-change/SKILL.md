...
4. **After creating an artifact, show progress**
   ...
   ```
   Then run `git status --short`, show the changed artifact files, and request explicit checkpoint approval before another artifact depends on this one.
...
- For proposal/design review, recommend `grill-with-docs` when material risk, assumptions, integration impact, or Brownfield context exists. Do not use the plain review skill.
...
- For ADR review, use `architectural-decision-records`; durable ADRs under top-level `adr/` are append-only and superseded by new ADRs, not rewritten.
...
**Guardrails**
- Checkpoint after the created artifact; do not stage, commit, merge, push, or archive without explicit user approval.
