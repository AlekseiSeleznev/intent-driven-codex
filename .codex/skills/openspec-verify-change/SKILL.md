...
3. **Get the change directory and load artifacts**
   ...
   This returns the change directory and `contextFiles` (artifact ID -> array of concrete file paths). Read all available artifacts from `contextFiles`. For intent-driven changes, also read top-level `adr/*.md` when present and derive the in-force ADR set by following supersession links.
...
- If full artifacts: verify all three dimensions, including ADR constraints when present
