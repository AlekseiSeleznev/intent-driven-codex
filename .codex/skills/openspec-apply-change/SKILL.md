...
3. **Git hard gate before apply**
   ...
   Refuse to implement if planning artifacts under `openspec/changes/<name>/` are missing or uncommitted (`proposal.md`, `specs/**`, `design.md`, `adr.md`, `tasks.md`) unless the user gives an explicit one-time override naming the files/gate.
...
5. **Read context files**
   ...
   - **intent-driven**: proposal, specs, design, per-change adr, tasks; then read top-level `adr/*.md` when present and derive the in-force ADR set by following supersession links
   - Other schemas: follow the contextFiles from CLI output

   For intent-driven changes, implementation must obey both the per-change ADR review and the in-force durable ADR set.
