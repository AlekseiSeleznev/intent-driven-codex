...
3. **Git hard gate before apply**
   ...
   Refuse to implement if planning artifacts under `openspec/changes/<name>/` are missing or uncommitted (`proposal.md`, `specs/**`, `design.md`, `review.md`, `adr.md`, `test-plan.md`, `tasks.md`) unless the user gives an explicit one-time override naming the files/gate.
...
4. **Get apply instructions**
   ...
   This returns:
   - `contextFiles`: artifact ID -> array of concrete file paths (varies by schema - could be proposal/specs/design/review/adr/test-plan/tasks or spec/tests/implementation/docs)
...
5. **Read context files**
   ...
   - **intent-driven**: proposal, specs, design, review, per-change adr, test-plan, tasks; then read top-level `adr/*.md` when present and derive the in-force ADR set by following supersession links
   - Other schemas: follow the contextFiles from CLI output

   For intent-driven changes, implementation must obey review resolutions, the per-change ADR review, test-plan verification ordering, and the in-force durable ADR set.
