...
поддержку архитектурных диаграмм, сценарии в стиле Gherkin, обязательные
контрольные точки Git и проверку совместимости надстройки.
...
- Жизненный цикл: `proposal -> specs -> design -> review -> adr -> test-plan -> tasks -> apply -> verify -> archive`.
...
- Обязательный gate `review.md` на основе `grill-with-docs`.
...
- Обязательный gate `test-plan.md` для TDD или максимально сильной verification-first замены.
...
`v0.1.2` включает внедрённые обязательные gates контекстного review и test-first verification:
...
- first-class артефакты `review.md` и `test-plan.md` в lifecycle `intent-driven`;
- `/opsx:continue`, `/opsx:apply`, `/opsx:verify` и overlay smoke checks, связанные с новыми gates `review`/`test-plan`;
...
проекта, формализация архитектуры project context и обязательные gates
`review`/`test-plan` уже заархивированы. В репозитории есть канонические
...
lifecycle gates `review`/`test-plan`. Сейчас активных изменений OpenSpec нет.
...
| Долговременные ADR проекта | `adr/0001-adopt-codex-native-intent-driven-openspec-overlay.md`, `adr/0003-formalize-project-context-entrypoints.md`, `adr/0004-adopt-mandatory-review-and-test-plan-gates.md` (ADR 0002 устарел и заменён ADR 0003) |
...
CONSTITUTION.md + ARCHITECTURE.md + ADRs"]
...
grill-with-docs findings"]
    TestGate["test-plan gate
TDD / verification-first strategy"]
...
включая review.md и test-plan.md"]
...
lifecycle и apply-context smoke tests"]
  ...
  class User actor;
  class Prompts,ConstitutionSkill,ReviewGate,TestGate,LifecycleSkills,GoalGuidance,GitSkill codex;
  class Constitution,Architecture,ADRs,SecretsExample rules;
...
| `review` | `review.md` | `design` | Обязательный контекстный review gate через `grill-with-docs`. |
| `adr` | `adr.md` | `review` | ADR review для изменения с учетом findings из review. |
| `test-plan` | `test-plan.md` | `adr` | TDD или verification-first план до задач реализации. |
| `tasks` | `tasks.md` | `specs`, `design`, `review`, `adr`, `test-plan` | Упорядоченный список задач реализации. |

Реализация начинается только после завершения `review`, `adr`, `test-plan` и `tasks` и фиксации состояния
планирования в Git.
...
рискованной реализации; он не заменяет `proposal.md`, спецификации, `design.md`, `review.md`,
...
| --- | --- | --- |
| Контекстная проверка изменения | `openspec/changes/<change>/review.md` | Обязательный gate `grill-with-docs` перед ADR. |
| Проверка ADR для изменения | `openspec/changes/<change>/adr.md` | Обязательный ADR gate с учетом findings из review. |
| Test plan изменения | `openspec/changes/<change>/test-plan.md` | TDD или verification-first план до tasks. |
