...
`v0.1.1` — текущая документированная версия шаблона. Она включает поддержку постоянного контекста проекта для OpenSpec workflow под управлением Codex:
...
- канонические спецификации OpenSpec и документацию по lifecycle, архитектуре контекста проекта, установке, update safety и secret handling.

## Текущее состояние репозитория

Начальное изменение реализации, подсказки Codex Goal, поддержка конституции
проекта и формализация архитектуры project context уже заархивированы. В
репозитории есть канонические спецификации OpenSpec для базовой надстройки,
goal-guided apply/bulk-apply, постоянного контекста проекта и текущего набора
проектных ADR.

| Область | Состояние |
| --- | --- |
| Активные изменения OpenSpec | нет (`openspec list --json` на этом checkpoint возвращает пустой список changes) |
...
| Архив изменений | `openspec/changes/archive/2026-05-24-implement-intent-driven-codex-template/`<br>`openspec/changes/archive/2026-05-24-add-codex-goal-guidance/`<br>`openspec/changes/archive/2026-05-25-add-project-constitution/`<br>`openspec/changes/archive/2026-05-26-formalize-project-context-architecture/` |
...
| Долговременные ADR проекта | `adr/0001-adopt-codex-native-intent-driven-openspec-overlay.md`, `adr/0003-formalize-project-context-entrypoints.md` (ADR 0002 устарел и заменён ADR 0003) |
...
tracked правила проекта, без секретов"]
  Architecture["ARCHITECTURE.md
текущий снимок архитектуры"]
  LocalSecrets[".secrets.local.env
локальные secret values, ignored"]
...
идеи workflow"]
  ...
  Codex -->|"использует prompts и skills"| Overlay
  Overlay -->|"запускает project context preflight"| Constitution
  Overlay -->|"читает текущую архитектуру"| Architecture
  Overlay -. "читает только когда нужен внешний доступ" .-> LocalSecrets
  Overlay -->|"вызывает публичный CLI"| OpenSpec
  OpenSpec -->|"читает и пишет OpenSpec artifacts"| TargetRepo
  Overlay -->|"устанавливает схему, skills, docs, constitution template"| TargetRepo
  Overlay -. "использует системы из constitution с local credentials" .-> ExternalSystems
  OpenSpec -. "не читает" .-> Constitution
...
создание, продолжение, реализация, проверка, sync, archive"]
...
обязательные checkpoints"]
  ...
  subgraph ProjectRules["Правила проекта и локальные доступы"]
    Constitution["CONSTITUTION.md
технологии, MCP, docs, verification"]
    Architecture["ARCHITECTURE.md
текущий снимок архитектуры"]
