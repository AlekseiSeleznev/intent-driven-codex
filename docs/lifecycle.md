...
OpenAI-compatible artifact review can also be controlled per session through ignored non-secret `.codex/session/openspec-session.json`, managed by `scripts/openspec-session-state` and the `/opsx:review-*` prompts. Ordinary `scripts/openspec-artifact-review --change <change> --stage <stage>` runs now use session-effective config by default: tracked `.codex/openspec-artifact-review.json` plus the current `artifactReview.decision`, provider overlay, and stage overlays. Explicit `--config <path>` remains deterministic and bypasses implicit session merge. Use `--raw-config` or `--ignore-session` to diagnose the tracked raw config directly.
