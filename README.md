...
- Optional Claude Code artifact review via `scripts/openspec-claude-review` and `.codex/openspec-claude-review.json`, with per-stage model/effort policy, optional budget caps, and structured reports.
...
`v0.1.6` includes the hardened Matt `grill-with-docs`/`tdd` lifecycle, Claude review/session/checkpoint controls, OpenSpec 1.4.0 auto-repair tooling hardening, and the installed-project repair UX fix archived on 2026-06-02:
...
Claude artifact review, automatic checkpoint/session controls, and Claude review
cost reporting, validation-report documentation fixes, and OpenSpec 1.4.0 auto-repair tooling hardening, and installed-project repair UX fix have all been archived. The repository contains canonical
...
grill/TDD gates, optional Claude review, and automatic safe checkpoint/session
controls.
...
| Archived changes | `openspec/changes/archive/2026-05-24-implement-intent-driven-codex-template/`<br>`openspec/changes/archive/2026-05-24-add-codex-goal-guidance/`<br>`openspec/changes/archive/2026-05-25-add-project-constitution/`<br>`openspec/changes/archive/2026-05-26-formalize-project-context-architecture/`<br>`openspec/changes/archive/2026-05-28-add-mandatory-review-and-tdd-gates/`<br>`openspec/changes/archive/2026-05-28-harden-mandatory-grill-and-tdd-gates/`<br>`openspec/changes/archive/2026-05-30-add-claude-artifact-review/`<br>`openspec/changes/archive/2026-05-30-add-default-git-automation-and-claude-session-controls/`<br>`openspec/changes/archive/2026-05-30-add-claude-review-cost-summary/`<br>`openspec/changes/archive/2026-05-30-fix-validation-report-findings/`<br>`openspec/changes/archive/2026-06-02-harden-openspec-140-tooling/`<br>`openspec/changes/archive/2026-06-02-fix-installed-overlay-repair-ux/` |
...
.codex/session, ignored and non-secret"]
  Review["Claude artifact review
optional claude -p reviewer"]
...
workflow ideas"]
  ...
  Session -->|"overlays review settings"| Review
...
session Claude review settings"]
...
Claude reports with optional cost"]
...
| `/opsx:claude-review-status` | Show effective Claude artifact review session settings and global defaults. |
| `/opsx:claude-review-on` | Enable Claude artifact review for the current ignored non-secret session overlay. |
| `/opsx:claude-review-off` | Disable Claude artifact review for the current session. |
| `/opsx:claude-review-reset` | Clear session Claude review overrides so global defaults apply again. |
| `/opsx:claude-review-set` | Set per-stage Claude review model, effort, budget, or blocking options in the session overlay. |
...
Projects can enable a secondary Claude Code reviewer for selected OpenSpec stages by editing `.codex/openspec-claude-review.json`. The default config is safe for new installs: global `enabled` is `false`, so no Claude auth is required until a project opts in. Budget caps are also disabled by default (`maxBudgetUsd: null`) because Claude Code startup/cache costs can make small caps fail before a useful review is returned.

When enabled, Codex calls `scripts/openspec-claude-review --change <change> --stage <artifact>` after a configured artifact is generated. The helper sends a bounded, secret-safe bundle to `claude -p`, requests structured JSON output, and writes auxiliary reports under `openspec/changes/<change>/reviews/`. Reports inform later gates and verify, but never replace OpenSpec validation, `grill.md`, `design-review.md`, ADR review, `test-plan.md`, or TDD evidence.

Use full model IDs such as `claude-sonnet-4-6` or `claude-opus-4-8` when reproducibility matters. Floating aliases such as `sonnet`, `opus`, and `haiku` are supported only when a project intentionally wants provider defaults. Balanced defaults use Sonnet for routine checks and Opus high effort for `design-review`, `adr`, and `verify`; budget caps and `xhigh`/`max` should be explicit opt-ins for costly reviews.

When an explicit budget cap is exhausted, Claude Code returns a budget error result. The review helper treats that as blocking, reports the exhausted budget, and automatically disables Claude review for the current session. Raise or clear the cap and explicitly re-enable review before retrying.

## Claude review session controls

Use `/opsx:claude-review-status`, `/opsx:claude-review-on`, `/opsx:claude-review-off`, `/opsx:claude-review-reset`, and `/opsx:claude-review-set` to manage local non-secret Claude artifact review settings for one Codex/OpenSpec session. Settings live under ignored `.codex/session/` and merge over `.codex/openspec-claude-review.json`; they do not store credentials and do not replace mandatory OpenSpec, grill, ADR, TDD, or verification gates. Set `budget=none` / `maxBudgetUsd=none` to remove a session budget cap.
...
.codex/
  ...
  openspec-claude-review.json      optional non-secret Claude review config
  session/                         ignored local workflow session state
adr/
  README.md                        ADR policy and index
  0001-*.md ... 0007-*.md          project architecture decisions
openspec/
...
scripts/
  ...
  openspec-claude-review           optional Claude artifact review helper
  scripts/openspec-session-state   ignored session overlay helper
  scripts/openspec-git-checkpoint  scoped local checkpoint helper
...
Current release: `v0.1.6`. See [`CHANGELOG.md`](CHANGELOG.md).
