...
project-local OpenSpec schema, ADR guidance, architecture diagram support,
Gherkin-style specs, mandatory Git checkpoints, and overlay smoke checks.
...
- Lifecycle: `proposal -> specs -> design -> review -> adr -> test-plan -> tasks -> apply -> verify -> archive`.
...
- Mandatory `review.md` gate powered by `grill-with-docs`.
...
- Mandatory `test-plan.md` gate for TDD or the strongest verification-first substitute.
...
`v0.1.2` includes the implemented mandatory context-aware review and test-first verification gates:
...
- first-class `review.md` and `test-plan.md` artifacts in the `intent-driven` lifecycle;
- `/opsx:continue`, `/opsx:apply`, `/opsx:verify`, and overlay smoke checks wired to the new review/test-plan gates;
...
project context architecture formalization, and mandatory review/test-plan gates
...
context, the current project ADR set, and the implemented review/test-plan
lifecycle gates. There are currently no active OpenSpec changes.
...
| Durable project ADRs | `adr/0001-adopt-codex-native-intent-driven-openspec-overlay.md`, `adr/0003-formalize-project-context-entrypoints.md`, `adr/0004-adopt-mandatory-review-and-test-plan-gates.md` (ADR 0002 is superseded) |
...
including review.md and test-plan.md"]
...
| `review` | `review.md` | `design` | Mandatory `grill-with-docs` context review gate. |
| `adr` | `adr.md` | `review` | Per-change ADR review gate informed by review findings. |
| `test-plan` | `test-plan.md` | `adr` | TDD or verification-first plan before implementation tasks. |
| `tasks` | `tasks.md` | `specs`, `design`, `review`, `adr`, `test-plan` | Dependency-ordered implementation checklist. |

Implementation starts only after `review`, `adr`, `test-plan`, and `tasks` are complete and the planning state has
been checkpointed.
...
never replaces `proposal.md`, specs, `design.md`, `review.md`, `adr.md`, `test-plan.md`, `tasks.md`,
...
| --- | --- | --- |
| Per-change context review | `openspec/changes/<change>/review.md` | Mandatory `grill-with-docs` gate before ADR. |
| Per-change ADR review | `openspec/changes/<change>/adr.md` | Required ADR gate informed by review findings. |
| Per-change test plan | `openspec/changes/<change>/test-plan.md` | TDD or verification-first plan before tasks. |
