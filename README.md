...
- Optional OpenAI-compatible artifact review via `scripts/openspec-artifact-review` and `.codex/openspec-artifact-review.json`, with configurable provider URL, model, effort/reasoning policy, optional request-provider routing, optional budget caps, and structured reports.
...
`v0.1.9` includes the hardened Matt `grill-with-docs`/`tdd` lifecycle, provider-neutral OpenAI-compatible review/session/checkpoint controls, OpenSpec 1.4.0 auto-repair tooling hardening, the installed-project repair UX fix archived on 2026-06-02, the OpenAI-compatible review replacement archived on 2026-06-03, Polza DeepSeek request-provider routing, full artifact-review cost ledger totals across retries/re-runs, formal review finding dispositions, compact localized review status with RUB costs rounded upward to kopecks, and refreshed README/C4/hero documentation for the current architecture:
...
- optional provider-neutral artifact review through `scripts/openspec-artifact-review` with safe disabled defaults, Polza-compatible non-secret defaults, session-level `/opsx:review-*` controls, structured cost-aware reports, formal finding dispositions, compact localized status output, and request-body provider routing for Polza/OpenRouter-compatible routers;
...
auto-repair tooling hardening, installed-project repair UX fix, and artifact-review disposition/status hardening have all been archived. The repository contains canonical
...
| Archived changes | `openspec/changes/archive/2026-05-24-implement-intent-driven-codex-template/`<br>`openspec/changes/archive/2026-05-24-add-codex-goal-guidance/`<br>`openspec/changes/archive/2026-05-25-add-project-constitution/`<br>`openspec/changes/archive/2026-05-26-formalize-project-context-architecture/`<br>`openspec/changes/archive/2026-05-28-add-mandatory-review-and-tdd-gates/`<br>`openspec/changes/archive/2026-05-28-harden-mandatory-grill-and-tdd-gates/`<br>`openspec/changes/archive/2026-05-30-add-claude-artifact-review/`<br>`openspec/changes/archive/2026-05-30-add-claude-review-cost-summary/`<br>`openspec/changes/archive/2026-05-30-add-default-git-automation-and-claude-session-controls/`<br>`openspec/changes/archive/2026-05-30-fix-validation-report-findings/`<br>`openspec/changes/archive/2026-05-30-move-readme-claude-review-controls/`<br>`openspec/changes/archive/2026-05-30-require-user-facing-claude-review-block/`<br>`openspec/changes/archive/2026-06-02-harden-openspec-140-tooling/`<br>`openspec/changes/archive/2026-06-02-fix-installed-overlay-repair-ux/`<br>`openspec/changes/archive/2026-06-03-replace-claude-review-with-openai-compatible-review/`<br>`openspec/changes/archive/2026-06-04-add-artifact-review-dispositions/`<br>`openspec/changes/archive/2026-06-05-fix-artifact-review-session-config/` |
...
Reviewer findings are not accepted blindly: Codex must independently handle each material finding as fixed, deferred to a concrete artifact/file/section/task, non-actionable with a reason, or user-blocked. Formal disposition files can be stored next to reports as `openspec/changes/<change>/reviews/<stage>-artifact-review-disposition.json`, and local validation can summarize whether any findings remain open. Live provider-call costs are appended to `openspec/changes/<change>/reviews/artifact-review-costs.jsonl`, and `scripts/openspec-artifact-review --cost-summary --change <change>` reports the full RUB total across retries/re-runs, with display values rounded upward to kopecks. In chat, Codex renders a compact localized `---`-wrapped status block instead of a verbose raw report.

The tracked default provider is Polza-compatible and non-secret: `baseUrl: https://polza.ai/api/v1`, `apiKeyEnv: POLZA_AI_API_KEY`, model `deepseek/deepseek-v4-pro`, and request-body routing `requestProvider: {"only":["DeepSeek"], "allow_fallbacks": false}` so the default review route does not fall back to more expensive providers. Store the actual API key only in ignored `.secrets.local.env` or the local environment.

Use `/opsx:review-status`, `/opsx:review-on`, `/opsx:review-off`, `/opsx:review-reset`, and `/opsx:review-set` to manage local non-secret artifact review settings for one Codex/OpenSpec session. Settings live under ignored `.codex/session/` and merge over `.codex/openspec-artifact-review.json`; ordinary `scripts/openspec-artifact-review --change <change> --stage <stage>` runs use this session-effective config by default. Explicit `--config <path>` stays deterministic and does not merge session state; use `--raw-config` / `--ignore-session` to inspect the tracked raw config directly. Session settings do not store credentials and do not replace mandatory OpenSpec, grill, ADR, TDD, or verification gates. Set `budget=none` / `maxBudgetUsd=none` or `budgetRub=none` / `maxBudgetRub=none` to remove a session budget cap. Use `providerOnly=DeepSeek allowFallbacks=false` to pin a Polza route for a session stage, or `requestProvider='{"only":["DeepSeek"],"allow_fallbacks":false}'` for the raw request-body routing object.
...
Current published release: `v0.1.9`. See [`CHANGELOG.md`](CHANGELOG.md).
